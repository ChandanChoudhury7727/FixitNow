...
export default function Register() {
  ...
  // -------------------------------------
  // 📍 Capture GPS Location + Reverse Geocode
  // -------------------------------------
  const captureLocation = async () => {
    if (!navigator.geolocation) {
      setMsg("❌ Geolocation not supported by your browser");
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        if (accuracy > 5000) {
          setMsg("⚠️ GPS accuracy is low. You can still edit the location manually.");
        }
        ...
        try {
          ...
          setMsg(`📍 Location detected: ${locationName}`);
        } catch (err) {
          ...
          setMsg("📍 Location captured (address unavailable)");
        } finally {
          setLoadingLocation(false);
        }
      },
      (err) => {
        ...
        switch (err.code) {
          case err.PERMISSION_DENIED:
            errorMsg += "Please enable location permission in your browser.";
            break;
          case err.POSITION_UNAVAILABLE:
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setMsg(serverMsg ? `❌ ${serverMsg}` : "❌ Registration failed");
    }
  };
          ...
          <div className="p-8">
            <form onSubmit={submit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <input
                  className="w-full border-2 border-gray-300 px-4 py-3 rounded-xl focus:ring-4 focus:ring-purple-100 focus:border-purple-500 outline-none transition-all duration-200"
                  placeholder="Enter your full name"
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  required
              ...
              <div className="space-y-2">
                ...
                <select
                  className="w-full border-2 border-gray-300 px-4 py-3 rounded-xl focus:ring-4 focus:ring-purple-100 focus:border-purple-500 outline-none"
                  value={form.role}
                  onChange={(e) => setForm({ ...form, role: e.target.value })}
                >
                  <option value="CUSTOMER">Customer</option>
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  onClick={captureLocation}
                  disabled={loadingLocation}
                  className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-5 py-3 rounded-xl font-semibold shadow-md hover:shadow-lg transition-all duration-200"
                >
                  {loadingLocation ? "Detecting..." : "Capture Location"}
                </button>
                ...
                {form.location && (
                  <input
                    ...
                    className="w-full border-2 border-gray-300 px-4 py-2 rounded-xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all duration-200"
                  />
                )}
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3.5 rounded-xl font-bold text-lg transition-all duration-200 hover:shadow-xl transform hover:scale-[1.02]"
              >
                Register
              </button>
              ...
              {msg && (
                <div
                  className={`text-sm p-4 rounded-xl border-2 mt-4 ${
                    msg.startsWith("✅")
                      ? "bg-green-50 border-green-200 text-green-700"
                      : msg.startsWith("📍")
                      ? "bg-blue-50 border-blue-200 text-blue-700"
